    ...
    <div className='flex justify-end space-x-2 mt-5'>
      ...
      {[
        ...
        <div key={id} className='flex flex-col items-center group'>
          <button onClick={action} className={`text-white font-bold py-2 px-4 rounded flex items-center space-x-2 transition-transform transform lg:hover:scale-105 ${colorClasses[id]}`}>
            {icon}
          </button>
          <span className={`text-sm text-white/70 mt-2 opacity-100 lg:opacity-0 transition-opacity duration-300 lg:group-hover:opacity-100`}>{label}</span>
        </div>
      ))}
    </div>
  )
