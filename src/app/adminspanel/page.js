...
export default function EventsPage() {
      ...
      const handleSubmit = async (e) => {
        ...
        const eventDetails = Eventsday.find((event) => event.name === selectedEvent);
        ...
        const validPOCs = eventDetails.contact.map((contact) => {
        ...
        if (!validPOCs.includes(pocNumber)) {
          alert("The entered POC number does not match any contact for this event.");
          return;
        }
  ...
  isdatafetched ? (
    ...
    <div className="max-w-4xl mx-auto py-8">
       ...
       <h1 className="text-6xl md:text-7xl text-center font-bold tracking-normal mb-4 sm:mb-12">
              ...
              Registered Teams for <span className=" dw decoration-fuchsia-500">{responseData.eventName}</span>
              </span>
            </h1>

      <div className="bg-white border-2  border-gray-300 rounded-lg p-4 text-center max-w-md mx-auto mb-6">
  <h2 className="text-3xl font-bold text-black">
    Total Registrations = {responseData?.teamsRegistered?.length || 0}
  </h2>
</div>
  ...
  <div key={team._id} className="bg-white p-8 rounded-lg shadow-md border border-gray-300">
    <h3 className="text-2xl font-bold text-black mb-6">
      {index + 1}. {team.teamName}
    </h3>

    <div className="mb-4 flex gap-x-2">
      <span className="text-lg font-semibold text-black">👤 Leader:</span>
      <span className="text-lg font-semibold text-black">{team.leaderName}</span>
    </div>

    <div className="mb-4 flex gap-x-2">
      <span className="text-lg font-semibold text-black">📞 Contact:</span>
      <a href={`tel:${team.leaderMobileNumber}`} className="text-lg font-semibold text-blue-500 hover:underline">
        {team.leaderMobileNumber}
      </a>
    </div>

    <div className="flex gap-x-2">
      <span className="text-lg font-semibold text-black">🎓 Roll No:</span>
      <span className="text-lg font-semibold text-black">{team.rollNumber}</span>
    </div>
  </div>
))}
      ...
      >
        <h1 className="text-6xl md:text-7xl text-center font-bold tracking-normal mb-4 sm:mb-12">
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#EFCA4E] via-[#F6F1E2] to-[#EFCA4E]">
            ...
            <span className="dw decoration-fuchsia-500">for Your Events</span>
          </span>
        </h1>
        ...
        {selectedClub && (
          <>
            ...
            >
              ...
              {Eventsday.filter((event) => event.club === selectedClub).map((event) => (
