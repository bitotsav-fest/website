...
export default function ChatbotPopup() {
  ...
  const questions = ["What events are conducted by IEEE?", "Whom should I contact for Mr and Ms Pantheon?", "When is Bitotsav going to happen?", "What is the category of Dance Saga?"]
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setChatHistory((prev) => [...prev.slice(0, -1), { type: "bot", text: "", isLoading: false }])
      ...
      setChatHistory((prev) => [...prev.slice(0, -1), { type: "bot", text: "Sorry, something went wrong. Please try again.", isLoading: false }])
    } finally {
      setLoading(false)
  ...
  useEffect(() => {
    if (isPopupOpen && isInitialRender.current) {
      ...
      setChatHistory([{ type: "bot", text: "Hi! I am BitoMind. You can ask me anything regarding the events here. Feel free to share your doubts." }])
    ...
    <>
      ...
      {!isPopupOpen && (
        <button
          ...
          className={`fixed bottom-8 right-8 z-50 bg-[#E18E04]/20 text-[#FCE2BF] font-bold rounded-full border-2 border-[#E18E04]/50 backdrop-blur-md hover:bg-[#E18E04]/40 hover:shadow-neon-orange focus:outline-none focus:shadow-neon-orange disabled:bg-[#a53302]/20 disabled:cursor-not-allowed flex items-center justify-center gap-2 group p-4 chatbot-toggle-button transition-all duration-300 ${
            isPopupOpen ? "rotate-90" : "rotate-0"
          }`}
        >
          <img src='chat-bot.png' alt='Chatbot Icon' className='w-10 h-10' />
        </button>
      )}

      {/* Chatbot Popup */}
      <div
        className={`fixed bottom-0 right-0 w-full lg:w-[400px] h-[calc(100%-60px)] bg-[#0A0118]/70 backdrop-blur-lg border-2 border-[#E18E04]/20 rounded-tl-3xl rounded-tr-3xl lg:rounded-tr-none lg:rounded-bl-3xl shadow-2xl overflow-hidden transform transition-transform duration-300 ${
      ...
      >
        ...
        <div className='w-full h-full flex flex-col items-center p-6'>
          <div className='w-full flex justify-center items-center mb-4 pt-4'>
            <h1 className='text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#EFCA4E] via-[#F6F1E2] to-[#EFCA4E]'>Ask Me Anything</h1>
          </div>

          {/* Chat History with Texting Bubbles */}
          <div ref={chatHistoryContainerRef} className='w-full mb-6 space-y-2 overflow-y-auto flex-1 px-4 scrollbar' style={{ maxHeight: "calc(100vh - 260px)" }}>
            {chatHistory.map((message, index) => (
              <div key={index} className={`flex ${message.type === "user" ? "justify-end" : "justify-start"}`}>
                {message.isLoading ? (
                  ...
                  <div
                    className={`max-w-[70%] p-4 rounded-2xl text-xs ${
                      message.type === "user"
                        ? "bg-[#813C01]/20 border-2 border-[#813C01]/50 text-[#FCE2BF] shadow-neon-red mt-6 appearance-none"
                        : "bg-[#E18E04]/20 border-2 border-[#E18E04]/50 text-[#FCE2BF] shadow-neon-orange mt-6 appearance-none"
          ...
          <div className='w-full flex flex-col md:flex-row gap-4 mb-4'>
            ...
            >
              {loading ? (
                ...
                <>
                  <span>Ask</span>
                  <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='currentColor' className='w-5 h-5 jiggle-animation'>
          ...
          {/* Error Message */}
          {error && <p className='text-red-400 mt-4 text-center font-semibold neon-text-red'>{error}</p>}
        </div>
      </div>
